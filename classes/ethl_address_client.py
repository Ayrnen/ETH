...
class ETHLAddressClient:
    ...
    def get_lp_position(self, pair):
        lp_address = self.config.get_value_checksum('mainnet-uniswap-pools', pair)
        lp_abi = self.etherscan.get_contract_abi(lp_address)
        lp_contract = self.web3.eth.contract(address=lp_address, abi=lp_abi)
